...
VALUES ((SELECT user_id FROM user_account WHERE user_role = 'admin'), 'admin', 'hashed_password', 'salt_value', 'bcrypt', 'admin@example.com', 'confirmed');
...
VALUES
    ((SELECT user_id FROM user_account WHERE user_role = 'admin'),
     (SELECT external_provider_id FROM external_providers WHERE provider_name = 'Google'),
     'google_token_12345'),
    ((SELECT user_id FROM user_account WHERE user_role = 'admin'),
     (SELECT external_provider_id FROM external_providers WHERE provider_name = 'GitHub'),
     'github_token_67890');

-- Create sample data for todos
INSERT INTO todos (title, deadline, completed, user_id)
VALUES
    ('Buy groceries', NOW() + INTERVAL '1 day', FALSE, (SELECT user_id FROM user_account WHERE user_role = 'admin')),
    ('Complete project report', NOW() + INTERVAL '2 days', FALSE, (SELECT user_id FROM user_account WHERE user_role = 'admin')),
    ('Schedule dentist appointment', NOW() + INTERVAL '5 days', TRUE, (SELECT user_id FROM user_account WHERE user_role = 'admin')),
    ('Call mom', NOW() + INTERVAL '3 days', TRUE, (SELECT user_id FROM user_account WHERE user_role = 'admin'));